...
const Step4Optimize = ({ data, updateData, setNextDisabled, onNext, onBack }) => {
    ...
    return (
        <div style={{ maxWidth: '800px', margin: '0 auto', width: '100%', textAlign: 'center' }}>
            ...
            {/* Mode selection */}
            <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1fr) minmax(0, 1fr)', gap: '1rem', marginBottom: '2.5rem', maxWidth: '700px', margin: '0 auto 2.5rem' }}>
                <div
                ...
                {!completed ? (
                    <motion.div key="action" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                        <button
                            ...
                            className="glow-btn"
                            style={{ fontSize: '1.1rem', padding: '1rem 3rem', background: 'var(--gradient-primary)', display: 'inline-flex', alignItems: 'center', gap: '0.75rem' }}
                        >
                            {isImproving ? (
                                <><Loader2 className="animate-spin" size={20} /> Optimizing Resume...</>
                            ) : (
                                <><Sparkles size={20} /> Run {improveMode === 'optimize' ? 'Optimization' : 'Regeneration'}</>
                            )}
                        </button>

                        <p style={{ fontSize: '0.85rem', color: 'var(--text-faint)', marginTop: '1rem' }}>
                            You will be able to review and tweak the changes in the next step.
                        </p>
                    </motion.div>
                ) : (
                    <motion.div key="success" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} style={{ padding: '2rem', background: 'rgba(16,185,129,0.05)', border: '1px solid rgba(16,185,129,0.2)', borderRadius: 'var(--radius-lg)', display: 'inline-block' }}>
                        <CheckCircle size={48} style={{ color: 'var(--success-light)', margin: '0 auto 1rem' }} />
                        <h3 style={{ fontSize: '1.4rem', fontWeight: '700', color: 'var(--success-light)', marginBottom: '0.5rem' }}>Optimization Complete!</h3>
                        <p style={{ color: 'var(--text-muted)' }}>We've successfully updated your resume for ATS systems.</p>
                    </motion.div>
                )}
            </AnimatePresence>
